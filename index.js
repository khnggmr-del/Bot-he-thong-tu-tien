...
function layTyLeDotPha(nhanh, idx, p) {
    ...
    if (p.congPhapDangTu === "cp_daodaohongtran" && laTrenTienDe(p)) tyLe += 100; 
    return tyLe;
}
...
function layTocDoBeseQuan(p) {
    ...
    if (p.theChat.includes("Phàm Trần Nghịch Mệnh Thể")) {
        if (laGiaiDoanPhamDenHoaThan(p)) spd *= 3.5; 
        else if (laGiaiDoanLuyenHuTroLen(p)) spd *= 5.5; 
    }
    ...
    if (p.congPhapDangTu && CONG_PHAP_BOOK[p.congPhapDangTu]) {
        ...
        if (p.congPhapDangTu === "cp_daodaohongtran" && laTrenTienDe(p)) {
            ...
            if (p.linhCanTen.includes("Thiên Linh Căn") || p.linhCanTen.includes(cp.he)) buffGoc *= 2; 
            spd += buffGoc;
        }
    }
...
function tuXoaTinNhan(msg, thoiGian = 20000) {
    if (msg && typeof msg.delete === 'function') setTimeout(() => msg.delete().catch(() => {}), thoiGian);
}

// ==================== HÀM BẾ QUAN & ĐỘT PHÁ (XỬ LÝ TOÀN DIỆN) ====================
async function batDauBeQuan(message, userId, isInteraction = false, interaction = null) {
    ...
    const interval = setInterval(async () => {
        ...
        if (p.tuViHienTai >= maxTv) {
            p.tuViHienTai = maxTv; p.nhiemVu.bqLan += 1; dangBeQuan.delete(userId); luuDuLieu(); clearInterval(interval);
...
async function handleDotPha(message, userId, isInteraction = false, interaction = null) {
    ...
    if (p.nhanhDao === "chinh" && p.chiSoCanhGioi === nhanhChinh.length - 1) {
        const luaChonMenu = new StringSelectMenuBuilder().setCustomId('lua_chon_dai_dao').setPlaceholder('Chọn Con Đường Chứng Đạo Chí Cao...')
            .addOptions(
                new StringSelectMenuOptionBuilder().setLabel('🌸 Hồng Trần Tiên (Hóa Phàm Luyện Tâm)').setDescription('Trở lại làm Phàm Nhân tu luyện').setValue('path_hongtran'),
                new StringSelectMenuOptionBuilder().setLabel('🌌 Vô Thượng Đại Đạo (Nghịch Chuyển Thiên Không)').setDescription('Tiến thẳng vào Vô Thượng Cảnh').setValue('path_vothuong')
            );
        const row = new ActionRowBuilder().addComponents(luaChonMenu);
        const txtChuyenNhanh = `🚨 **<@${userId}> CẢNH BÁO THIÊN ĐẠO!** Đạo hữu đã chạm tới đỉnh phong **Tiên Đế**. Chọn ngã rẽ định mệnh:`;
    ...
    let chuoiDan = ""; let hoMach = false; const cgTen = layTenCanhGioi(p.nhanhDao, p.chiSoCanhGioi);
    ...
    let checkNhayCap = (tyLeThucTe >= 200); let tyLeXacSuat = Math.min(tyLeThucTe, 100); p.nhiemVu.dpLan += 1;
    let thongBaoGoc;
    
    if (isInteraction) {
        await interaction.reply({ content: `⚡ Quy tắc chi lực tụ tập... Hãy đợi kết quả sau 2 giây!`, ephemeral: true });
    } else {
        thongBaoGoc = await message.channel.send(`⚡ Thiên địa biến sắc! Kiếp lôi giáng xuống <@${userId}>... Vận khí: **${tyLeThucTe}%** ${chuoiDan ? `\n Phụ trợ: ${chuoiDan}` : ""}`);
    }
    ...
    setTimeout(() => {
        ...
        if (Math.random() * 100 <= tyLeXacSuat) {
            let soCapTang = checkNhayCap ? 2 : 1; p.chiSoCanhGioi += soCapTang; p.tuViHienTai = 0;
            let mangCanhGioiHienTai = p.nhanhDao === "vothuong" ? nhanhVoThuong : p.nhanhDao === "hongtran" ? nhanhHongTran : nhanhChinh;
            if (p.chiSoCanhGioi >= mangCanhGioiHienTai.length) p.chiSoCanhGioi = mangCanhGioiHienTai.length - 1;

            let textThànhCông = checkNhayCap 
                ? `🔥 **[NGHỊCH THIÊN NHẢY CẤP]** Tôn thượng <@${userId}> thân thể tích lũy đại bộc phát, **NHẢY VỌT LIỀN 2 TIỂU CẢNH GIỚI** tiến tới **${layTenCanhGioi(p.nhanhDao, p.chiSoCanhGioi)}**!`
                : `🎉 **[THÀNH CÔNG]** Chúc mừng tôn thượng <@${userId}> đột phá lên vị trí **${layTenCanhGioi(p.nhanhDao, p.chiSoCanhGioi)}**!`;
            if (thongBaoKenh) thongBaoKenh.send(textThànhCông).then(m => tuXoaTinNhan(m));
        } else {
            let phanTram = hoMach ? 0.15 : 0.50; p.tuViHienTai -= Math.floor(p.tuViHienTai * phanTram);
            dangDuongThuong.set(userId, Date.now() + 20000); 
            const userObj = isInteraction ? interaction.user : message.author;
            userObj.send(`💥 **[THẤT BẠI]** Đột phá thất bại, tổn thất ${(phanTram*100)}% tu vi và chấn thương kinh mạch 20 giây!`).catch(()=>{});
        }
        luuDuLieu();
    }, 2000);
}
...
client.on('messageCreate', async (message) => {
    ...
    if (cmd === '!npt' && userId === VIP_USER_ID) {
        ...
        databaseTuTien[userId].tuiDo['cp_daodaohongtran'] = 1; luuDuLieu();
        return message.author.send(`🌌 **[VIP KHỞI ĐỘNG]** Kích hoạt Tiên Đế Thần Thể vĩnh hằng thành công!`).catch(()=>{});
    }
    ...
    if (cmd === '!hethong') {
        ...
        if (p.linhCanTen === "Chưa thức tỉnh") {
            ...
            if (!kiemTraMaxDaiDao(p)) {
                ...
                menu.addOptions(new StringSelectMenuOptionBuilder().setLabel('🌌 Đại Đạo Chí Cao').setValue('disabled'));
    ...
    if (cmd === '!shop') {
        ...
        const menuCongPhap = new StringSelectMenuBuilder().setCustomId('menu_shop_congphap').setPlaceholder('Mua Bí Kíp Công Pháp...')
        ...
        return message.channel.send({ 
            content: `### 🏪 **[THIÊN BẢO CÁC]**\nTài sản: **${p.linhThach}** LT. Chọn vật phẩm cần mua:`, 
    ...
    if (cmd === '!tuido') {
        let txt = `### 🎒 **[TÚI TRỮ VẬT]**\n`; let hangNutDan = new ActionRowBuilder(); let hangNutCp = new ActionRowBuilder();
        Object.entries(SHOP_ITEMS).forEach(([k, i]) => { if (p.tuiDo[k] > 0) txt += `• **${i.ten}**: \`${p.tuiDo[k]} viên\`\n`; });
        ...
        Object.entries(CONG_PHAP_BOOK).forEach(([k, i]) => {
            if (p.tuiDo[k] > 0) {
                coCp = true; txt += `• **${i.ten}**: \`Chưa luyện\`\n`;
                if (hangNutCp.components.length < 5) hangNutCp.addComponents(new ButtonBuilder().setCustomId(`learn_${k}`).setLabel(`Tu luyện ${i.ten.split(' ')[0]}`).setStyle(ButtonStyle.Success));
            }
        });
        if (!coCp) txt += `_Trống trơn._\n`;
        const components = [];
        if (hangNutDan.components.length > 0) components.push(hangNutDan);
        if (hangNutCp.components.length > 0) components.push(hangNutCp);
        return message.channel.send({ content: txt, components }).then(m => tuXoaTinNhan(m));
    }
    ...
    if (cmd.startsWith('!ketduyen')) {
        ...
        return message.channel.send(`💞 **[THIÊN ĐẠO CHỨNG GIÁM]** Đã kết thành đạo lữ!`).then(m => tuXoaTinNhan(m));
    }
    if (cmd === '!nhiemvu') {
...
// ==================== INTERACTION XỬ LÝ SỰ KIỆN NÚT BẤM VÀ MENU ====================
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isStringSelectMenu()) {
        ...
        if (interaction.customId === 'menu_shop_congphap') {
            ...
            if (p.tuiDo[cpId] > 0 || p.congPhapDangTu === cpId) return interaction.reply({ content: `⚠️ Đạo hữu đã có công pháp này!`, ephemeral: true });
            p.tuiDo[cpId] = 1; p.linhThach -= cpItem.gia; luuDuLieu();
            return interaction.reply({ content: `🛍️ Sở hữu thành công bí kíp **${cpItem.ten}**!`, ephemeral: true });
        }
            ...
            if (value === 'lcan') {
                ...
                if (Math.random() * 100 <= 15) {
                    if (!p.theChat.includes("Phàm Trần Nghịch Mệnh Thể")) {
                        ...
                        chuoiTheChatMoi = "\n🌸 **[DỊ TƯỢNG]** Huyết mạch thức tỉnh: **Phàm Trần Nghịch Mệnh Thể**!";
                    }
                }
                luuDuLieu();
    ...
    if (interaction.isButton()) {
        ...
        if (id === 'use_tkd' && p.tuiDo.tu_khi_dan > 0) { p.tuiDo.tu_khi_dan--; p.tuViHienTai = Math.min(p.tuViHienTai + 50, maxTv); luuDuLieu(); }
            ...
            if (p.tuiDo[cpId] > 0) {
                ...
                return interaction.reply({ content: `📜 Bạn đã bắt đầu vận hành công pháp: **${CONG_PHAP_BOOK[cpId].ten}**!`, ephemeral: true });
            }
        }
...
// SERVER GIỮ ĐƯỜNG TRUYỀN RENDER KHÔNG BỊ DIE PORT
